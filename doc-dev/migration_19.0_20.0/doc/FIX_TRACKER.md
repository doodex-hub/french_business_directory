...
| 2 | **MF-02** | SIRET kini disimpan di identifier tervalidasi Odoo 20 (SIRET tak valid ditolak, SIREN terisi otomatis) | Perubahan platform 20.0 | Tidak (kode sudah jadi) | A) **Terima validasi bawaan Odoo** (rekomendasi — sudah terbukti live dengan SIRET La Poste) · B) Matikan validasi seperti 19.0 (risiko error tersembunyi saat edit kontak) | ⏳ Menunggu keputusan A/B/C (klarifikasi: SIRET valid aman, dialog hanya untuk SIRET salah — dijawab 2026-09-25) | Live Pass S-07; SIRET tak valid → dialog Odoo "Invalid identifier: … (SIRET)", kontak tidak berubah |
| 3 | **MF-03** | Aturan tombol "Business Directory": sekarang tampil untuk semua kontak tanpa parent (workaround), termasuk individu | Perubahan platform 20.0 (`is_company` dari VAT) | Mungkin | A) **Sign-off workaround sekarang** (`invisible="parent_id"`) · B) Aturan lain yang dev tentukan (mis. sembunyikan untuk kontak yang jelas individu) | ⏳ Keputusan A/B/C (dev tanya kenapa individu dapat tombol — dijawab 2026-09-25) | AI cek 2026-09-25: DEMO 1/2/4 tombol tampil, DEMO 3 (anak) tersembunyi; data `10_qa/mf03_rmv04_demo_setup.py` |
