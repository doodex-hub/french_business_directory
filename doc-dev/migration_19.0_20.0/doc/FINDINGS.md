...
| MF-02 | `res.partner.company_registry` dihapus dari base 20.0 → SIRET pindah ke JSON `additional_identifiers['FR_SIRET']` (tervalidasi) | Step 1 (pre-scan) / 2 | `[GAP-MIGRASI]` | Kritis (fitur inti "Select") | ✅ CLOSED (keputusan dev 2026-09-25: opsi B) — SIRET via `additional_identifiers`, validasi native dipakai; SIRET tidak valid → UserError modul berbahasa Inggris, kontak tidak berubah |
| MF-03 | `base.view_partner_form` 20.0 dirombak: `<field id="company" name="name">` hilang, `is_company` jadi computed dari VAT | Step 1 (pre-scan) / 2 / 6 (G1 #6) | `[GAP-MIGRASI]` + `[PERLU-KEPUTUSAN]` (visibilitas tombol) | Tinggi (install-blocking + UX tombol) | ✅ CLOSED (keputusan dev 2026-09-25: opsi A, terima) — tombol `invisible="parent_id"`: semua kontak tanpa parent (termasuk individu) dapat tombol; kontak anak tidak. Diverifikasi live (DEMO 1-4) |
