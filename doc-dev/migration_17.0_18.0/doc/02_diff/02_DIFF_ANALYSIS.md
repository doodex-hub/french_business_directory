...
| DIFF-09 | `personal_email_usage/models/mail.py:6` — `from odoo.tools import logging` | `odoo/tools/__init__.py` (`from .misc import *`), `odoo/tools/misc.py` | **Behavior berubah** — 18.0 `misc.py` menambahkan `__all__` eksplisit yang TIDAK mencantumkan `logging`, menutup leak implisit stdlib `logging` yang sebelumnya (17.0, tanpa `__all__`) ikut ter-export lewat wildcard `from .misc import *`. `odoo.tools.logging` **tidak pernah menjadi API resmi** — ini efek samping tidak disengaja dari wildcard import 17.0 | **Critical — install-blocking, ditemukan lewat G1 dry run nyata (bukan review statis).** `ImportError: cannot import name 'logging' from 'odoo.tools'` saat modul di-load. Fix: ganti `from odoo.tools import logging` → `import logging` (stdlib langsung, tidak pernah bergantung ke re-export tidak resmi manapun) | Verifikasi langsung `native-source`/`native-target` (`odoo/tools/misc.py` baris `__all__`) + G1 dry run `french_business_directory_17_18-odoo_target-1`, 2026-08-24 |
