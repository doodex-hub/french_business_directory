...
| MF-13 | Tombol "Save manually" di form `fetchmail.server` (Odoo build `18.0-20260817`) tidak pernah benar-benar menyimpan record lewat Playwright MCP — klik terkirim (dikonfirmasi listener manual), tidak ada RPC/error/rejection apapun, dikonfirmasi record tidak tersimpan via `odoo shell` langsung | 10 (S-15, ditemukan 2026-08-31 lewat percobaan QA browser nyata) | `[GAP-MIGRASI]` (potensial — kemungkinan bug environment/Odoo build, BUKAN kode modul ini) | Sedang (menghalangi verifikasi persistence S-15, tidak menghalangi fungsi modul — `fetch_mail()` sendiri sudah terverifikasi penuh lewat test otomatis MF-11/MF-12) | 🔴 Belum terselesaikan — field `mark_read` sendiri dikonfirmasi ADA & posisi benar (bagian dari S-15 yang berhasil), tapi verifikasi "tersimpan setelah reload" tidak bisa dituntaskan. Butuh investigasi lanjutan (coba build Odoo 18.0 lain, atau observasi manual dev non-headless) sebelum ditutup |
...
**Rekomendasi:** investigasi lanjutan di luar scope migrasi modul ini — coba reproduce di build Odoo 18.0 lain (bukan nightly `20260817`), atau observasi manual dev sendiri di browser non-headless untuk exclude Playwright MCP sepenuhnya dari kemungkinan penyebab.
**Keputusan pemilik modul:** *(kosong — belum ada keputusan, S-15 langkah 4 dibiarkan terbuka)*
